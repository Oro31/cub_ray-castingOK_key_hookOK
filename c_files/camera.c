...
char	ft_fov_to_dir(float fov)
{
	...
	tmp = '\0';
	if (fov > M_PI / 2 - 0.00001 && fov < M_PI / 2 + 0.00001)
		tmp = 'N';
	else if (fov > 2 * M_PI - 0.00001 || fov < 0.00001)
		tmp = 'E';
	else if (fov > 3 * M_PI / 2 - 0.00001 && fov < 3 * M_PI / 2 + 0.00001)
		tmp = 'S';
	else if (fov > M_PI - 0.00001 && fov < M_PI + 0.00001)
		tmp = 'W';
	return (tmp);
}
